...
namespace Starwars
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            var Opgave1 = planets.Where(x => x.Name.StartsWith("M"));
            var Opgave2 = planets.Where(x => x.Name.ToLower().Contains("y"));
            var Opgave3 = planets.Where(x => x.Name.Length > 9 && x.Name.Length < 15);
            var Opgave4 = planets.Where(x => x.Name[1] == 'a' && x.Name.EndsWith("e"));
            var Opgave5 = planets.Where(x => x.RotationPeriod > 40).OrderBy(x => x.RotationPeriod);
            var Opgave6 = planets.Where(x => x.RotationPeriod > 10 && x.RotationPeriod < 20).OrderBy(x => x.Name);
            var Opgave7 = planets.Where(x => x.RotationPeriod > 30).OrderBy(x => x.Name).ThenBy(x => x.RotationPeriod);
            var Opgave8 = planets.Where(x => (x.RotationPeriod < 30 || x.SurfaceWater < 50) && x.Name.Contains("ba"))
                .OrderBy(x=> x.Name).ThenBy(x => x.SurfaceWater).ThenBy(x => x.RotationPeriod);
            var Opgave9 = planets.Where(x => x.SurfaceWater > 0).OrderByDescending(x => x.SurfaceWater);
            Program program = new Program();
            var Opgave10 = planets.Where(x => x.Diameter != 0 && x.Population > 0).ToList()
                .OrderBy(x => program.CalculateDensity(x.Diameter, x.Population));
            
            
            
            var newPlanets = planets.Except(planets.Where(x => x.RotationPeriod > 0)).ToList();
            
            
            
            
            //Opgave 12
            var opgave121 = planets.Where(x => x.Name.ToLower().StartsWith("a") || x.Name.ToLower().EndsWith("s"))
                .ToList();
            var opgave122 = planets.Where(x => x.Terrain != null && x.Terrain.Contains("rainforests")).ToList();
            var opgave12 = opgave121.Union(opgave122);

            
            
            
            //Opgave 13
            //var Task13 = planets.Where(x => x.Climate)

            //Opgave 14
            var task14 = planets.OrderBy(x => x.RotationPeriod).ThenBy(x => x.Name);


            //Opgave 15 
            var task15 = planets.Where(x => program.CheckForDoubleVowels(x.Name));
            
            //Opgave 16
            var task16 = planets.Where(x => program.CheckForDoubleKLROrN(x.Name)).OrderBy(x=> x.Name);
            Console.ReadKey();
        }
        ...
        private double CalculateDensity(double diameter, double population)
        {
            var t =(double)((Math.Pow(diameter / 2, 2) * Math.PI * 4) / population);
            Console.WriteLine(t);
            return t;
        }

        //public void intetNavn(Planet p)
        //{
        //    Regex.Match("N")
        //}
        private bool CheckForDoubleVowels(string name)
        {
            var match = Regex.Match(name, "[a]{2}|[e]{2}|[o]{2}|[i]{2}|[u]{2}|[y]{2}");
            return (match.Length > 0);
        }
        private bool CheckForDoubleKLROrN(string name)
        {
            var match = Regex.Match(name, "[k]{2}|[l]{2}|[r]{2}|[n]{2}");
            return (match.Length > 0);
        }
    }
}
